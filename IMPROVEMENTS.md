...
| **Подтверждение почты** | Основа заложена: колонки `email_verified_at`, `email_verification_token`, `email_verification_sent_at` в `users`; комментарии в `server.py` (register, заглушки `GET /auth/verify-email`, `POST /auth/send-verification-email`). Реализовать: отправка письма при регистрации, ссылка с token, эндпоинт верификации, опционально проверка `email_verified_at` перед чувствительными действиями. | Средняя |
...
| **Устройства безопасности** | Реализовано: при логине с `device_name` создаётся запись в `user_devices`, фронт хранит `device_id` и шлёт `X-Device-Id`. В настройках — список устройств, отзыв одного или всех кроме текущего. Дополнительно: при отзыве устройства можно инвалидировать токены (blacklist по device_id или JTI в токене). | Низкая (расширение) |
